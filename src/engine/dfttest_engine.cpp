...
std::unique_ptr<DftExecutor> create_dft_executor(
  std::string_view fft_backend,
  unsigned opt,
  ...
  if (fft_backend == "vkfft" || fft_backend == "vkfft-vulkan") {
    return create_vulkan_dft_executor(opt, format, backend.vulkan_runtime());
  }
#else
  (void)fft_backend;
#endif
