...
def run_avs_case(case: dict, plugin_path: Path, avs_dump: Path) -> list[dict]:
    ...
    script = render_avs_script(str(plugin_path), source, params)

    results = []
    with tempfile.TemporaryDirectory(prefix="neo-dfttest-baseline-") as temp_dir_name:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate or verify neo-dfttest frame hash baselines.")
